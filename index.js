...
async function scheduleMeeting(
  ...
  let suitableFreeSlot = commonFreeSlots.map(element => element.commonSlot)
...
if(suitableFreeSlot){
  ...
  if(sendListArray.length == 0){
    ...
    console.log("No common free sots found between the users, send Email to the email address " + sendListArray)
  }
}
...
function findCommonFreeSlots(startTime, endTime, busyPeriods, jsonDataFormate) {
  ...
  var startDate = moment.utc(data.start_date);
  var endDate = moment.utc(data.end_date);
  ...
  let arr=[]
  let officeStart=moment('2024-03-27T08:00:00.0000000')
  let officeEnd=moment('2024-03-27T18:30:00.0000000')
  let morningEveningTime=moment("2024-01-06T12:00:00")
  ...
  const time = moment(data.start_time, 'HH:mm:ss');
  
  // Merge date and time
  const mergedDateTime = date.add({
    hours: time.hours(),
    minutes: time.minutes(),
    seconds: time.seconds()
  });
  const time2 = moment(data.end_time, 'HH:mm:ss');
  const mergedDateTime2 = date.add({
    hours: time2.hours(),
    minutes: time2.minutes(),
    seconds: time2.seconds()
  });
  console.log(mergedDateTime.format('YYYY-MM-DDTHH:mm:ss.SSSSSSS'));
    fixedStart=mergedDateTime.format('YYYY-MM-DDTHH:mm:ss.SSSSSSS')
    fixedEnd=mergedDateTime2.format('YYYY-MM-DDTHH:mm:ss.SSSSSSS')
  }
  ...
  function getTime(users,duration,officeStart,officeEnd,morning,evening,fullDay,fixedStart,fixedEnd,newMorningEveningTime){
    ...
    users.forEach(user => {
      ...
      user['freeslotsinmin']=[]
        ...
        user['freeslotsinmin'].push(moment(officeEnd).diff(moment(officeStart)).asMinutes())
        }
  ...
  for (let index = 0; index < users.length; index++) {
      ...
      for (let j = 0; j < element.freeMinutes.length; j++) {
        ...
        group.push(element)
          ...
          let flag=false
          if(morning){
            if (startTimeElement.isBefore(moment(newMorningEveningTime)) && endTimeElement.isBefore(moment(newMorningEveningTime)) && checkDuration>=duration) {
              flag=true
            }
          } 
          if(evening){
...
app.post("/schedule-meeting", async (req, res) => {
  try {
    const { meeting_title,
      ...
      participants,
      meeting_location,
      meeting_duration } = req.body;

      let office_Start=moment('2024-03-27T08:00:00.0000000')
      let office_End=moment('2024-03-27T18:30:00.0000000')

      let start_Date_time = office_Start.clone().set({
        year: moment(start_date).year(),
        month: moment(start_date).month(),
        date: moment(start_date).date()
      });
      let end_Date_time = office_End.clone().set({
