...
async function scheduleMeeting(
  ...
  const startTimeAmsterdam = startTime.toLocaleString('en-US', { timeZone: amsterdamTimeZone });
const endTimeAmsterdam = endTime.toLocaleString('en-US', { timeZone: amsterdamTimeZone });
...
console.log(endTimeAmsterdam)
  ...
  const freeBusy = await authArray[0]
...
console.log("freeBusyResponses.value:----------")
console.log(freeBusy.value);

const busySlotsArray = freeBusy.value
  ...
  if (sendListArray.length == 0) {
    if (suitableSlots.length > 0) {
      ...
      // setMeeting(meeting_title, meeting_location, meeting_description, firstSlotStart, meeting_timeZone, meetingEndTime, calendarIds, calendars, authArray)

      // setMeeting(meeting_title, meeting_location, meeting_description, meeting_timeZone, calendarIds, calendars, authArray, scheduled_date, scheduled_start_time, scheduled_end_time, firstSlotStart, meetingEndTime)

      setMeeting(meeting_title, meeting_location, meeting_description, meeting_timeZone, calendarIds, authArray, scheduled_date, scheduled_start_time, scheduled_end_time, firstSlotStart, meetingEndTime)
...
// async function setMeeting(summary, meetingLocation, description, firstSlotStart, timeZone, meetingEndTime, calendarIds, calendars, authArray){

// async function  setMeeting(meeting_title, meeting_location, meeting_description, meeting_timeZone, calendarIds, calendars, authArray, scheduled_date, scheduled_start_time, scheduled_end_time, firstSlotStart, meetingEndTime){

async function  setMeeting(meeting_title, meeting_location, meeting_description, meeting_timeZone, calendarIds, authArray, scheduled_date, scheduled_start_time, scheduled_end_time, firstSlotStart, meetingEndTime){
...
function calculateCommonFreeSlots(startTime, endTime, busySlots, jsonData, calendarIds) {
  ...
  const freeBusyResponses = busySlots.map((element => element.scheduleItems))

  

  const freeBusyResponsesFlat = freeBusyResponses.flat()


  const TIMEZONE = busySlots.map((element => element.workingHours))

  console.log("TIMEZONE")
  console.log(TIMEZONE)


  console.log("freeBusyResponsesFlat:---------------")
  console.log(freeBusyResponsesFlat)

  const meetings = freeBusyResponsesFlat.map(response => {
    const startDateTime = response.start ? response.start.dateTime : 'Unknown';
    const endDateTime = response.end ? response.end.dateTime : 'Unknown';
...
// Convert each meeting time to Amsterdam time zone
const meetingsInAmsterdamTime = meetings.map(meeting => ({
    start: convertToAmsterdamTime(meeting.start),
    end: convertToAmsterdamTime(meeting.end)
}));
  ...
  console.log("meetingsInAmsterdamTime:---------");
  console.log(meetingsInAmsterdamTime);



  const busyPeriods = calendarIds.map(email => ({
    [email]: meetingsInAmsterdamTime.map(slot => ({
      start: slot.start,
      end: slot.end
    }))
  }));
...
if(morning){
  //startTimeElement.isBefore(moment(newMorningEveningTime)) && moment.duration(newMorningEveningTime.diff(startTimeElement)).asMinutes() >=duration
  if (closestTimeSlot.start.isBefore(moment(newMorningEveningTime)) && moment.duration(newMorningEveningTime.diff(commonStart)).asMinutes() >=duration) {
...
app.post("/send-auth-email", async (req, res) => {
  try {
    ...
    // console.log("sendAuthEmailRouteCalendarIds:---------")
    // console.log(calendarIds)


    const { participants } = req.body;

    // const calendarIds = participants.map(
    ...
    for (const participant of participants) {
      ...
      // console.log(`Authorization email sent to ${calendarId}`);
    }

    // const calendarIds = participants.map(participant => participant.email.trim());
    // console.log("sendAuthEmailRouteCalendarIds01:---------")
    // console.log(calendarIds)

    // if (calendarIds.length === 0) {
    //   return res.status(400).json({ error: "No email addresses found in participants data" });
    // }

    //     // console.log("sendAuthEmailRouteCalendarIds:---------")
    // // console.log(calendarIds)

    // console.log("calendarIds(users email addresses to send authentication emails):--------------")
    // console.log(calendarIds)
...
app.get('/auth/redirect/callback',
