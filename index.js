...
const { createToken, verifyToken } = require("./common/jwt");
...
async function adminLogin(data) {
  ...
  const result = await knex
    .select("admin_id", "name", "email", "last_updated", "created")
    .from(ADMIN)
...
async function adminLoginTest(data) {
  ...
  return false;
  await outh2CallBackTest(
    "4/0AeaYSHCPuEdtY120ZmE2ew9SS3VpEJfnFqQcIAErDPrqSegeFbc5-YatQV2YEv2c7gwB9w"
  );
  return false;
  return false;
  let bodydata = {
  ...
  let bodydata1 = {
    ...
    status: "1",
    sendEmail: [
      { email: "nickj7602@gmail.com", user_id: 4 },
      { email: "llita6005@gmail.com", user_id: 5 },
    ],
  };
...
async function getUsers(requestBody) {
  ...
  if (result.length > 0) {
    ...
    returnData = {
      body: JSON.stringify({
        message:
          "You seem to be no longer registered in the system, please contact system administrator and try again.",
        code: 401,
        data: [],
      }),
      headers: {
...
async function addMeeting(requestBody) {
  ...
  if (requestBody.participants) {
    ...
    try {
      ...
      let meetingAfterAddHandling = await handleMeetingStatus(
...
async function saveCurrentSelectionForMeeting(requestBody) {
  ...
  const result = await knex
    ...
    .where("meeting_activity_log_id", requestBody.activityLogId)
    .andWhere("user_id", requestBody.userId)
    .andWhere("is_updated", 0);
  console.log("result output");
  console.log(result);
  if (result.length > 0) {
    console.log("coming in log yes");
    let meetingAccessLogData = await getAccessLogData(
      result[0]["meeting_id"],
      requestBody.activityLogId
    );
    console.log(meetingAccessLogData);
    const allAccepted = meetingAccessLogData.every(
      (record) => record.accept_status === 1
    );

    // Check if any record has accept_status set to 4 (rejected)
    const anyRejected = meetingAccessLogData.some(
      (record) => record.accept_status === 4
    );

    if (allAccepted) {
      // Execute set meeting code
      console.log(
        "All users have accepted the meeting. Setting the meeting..."
      );
      // Call your set meeting function here
    } else if (anyRejected) {
      // Mark the meeting as rejected
      console.log(
        "At least one user has rejected the meeting. Marking the meeting as rejected..."
      );
      // Call your reject meeting function here
    } else {
      // Some users have not responded or their status is different than 1 or 4
      console.log(
        "Some users have not responded or their status is different than accepted or rejected."
      );
    }
    ...
    if (iterationCount <= 4) {
      ...
      if (requestBody.status == 4 || requestBody.status == "4") {
        ...
        let updateMeetingLogStatus = await updateMeetingLog(
          requestBody.activityLogId,
          {
        ...
        let meetingObjectNew = await getSingleMeetingData(
          result[0]["meeting_id"]
        );
        console.log("single meeting data for algo");
        console.log(meetingObjectNew);
        ...
        if (iterationCount < 4) {
          ...
          let scheduleJSON = {
            ...
            status: 4,
            message:
              "Reached maximum iteration count, but couldn't find a common time-slot(s) among meeting's participants.",
          };
        ...
        async function getUTCDateTime(
          ...
          const startDateTime = moment.utc(
            scheduledDate.format("YYYY-MM-DD") +
              "T" +
              adjustedStartTime.format("HH:mm:ss") +
              "Z"
          );
          const endDateTime = moment.utc(
            scheduledDate.format("YYYY-MM-DD") +
              "T" +
              adjustedEndTime.format("HH:mm:ss") +
              "Z"
          );
        ...
        if (allAccepted) {
          ...
          const participantEmails = meetingObjectNew[0].participants.map(
          ...
          const startTimeString = `${getProperDate(
            meetingObjectNew[0]["scheduled_date"]
          )}T${meetingObjectNew[0]["scheduled_start_time"]}.000Z`;
          const endTimeString = `${getProperDate(
            meetingObjectNew[0]["scheduled_date"]
          )}T${meetingObjectNew[0]["scheduled_end_time"]}.000Z`;
          let START = moment(
            getProperDate(meetingObjectNew[0]["scheduled_date"])
          )
            .clone()
            .hours(
              moment(
                meetingObjectNew[0]["scheduled_start_time"],
                "HH:mm:ss"
              ).hours()
            )
            .minutes(
          ...
          let END = moment(getProperDate(meetingObjectNew[0]["scheduled_date"]))
            .clone()
            .hours(
              moment(
                meetingObjectNew[0]["scheduled_end_time"],
                "HH:mm:ss"
              ).hours()
            )
            .minutes(
          ...
          let newStartEnd = await getUTCDateTime(
            meetingObjectNew[0]["scheduled_date"],
            meetingObjectNew[0]["scheduled_start_time"],
            meetingObjectNew[0]["scheduled_end_time"]
          );
          ...
          const participant = meetingObjectNew[0]["participants"].find(
            (participant) => participant.user_id == requestBody.userId
          );
      ...
      let returnData = {
        body: JSON.stringify({
          message:
            "The meeting has reached its maximum iteration limit, making scheduling unfeasible at this time. We advise contacting the administrator for further support and guidance.",
          code: 401,
          data: [],
        }),
        headers: {
    ...
    let returnData = {
      body: JSON.stringify({
        message:
          "Meeting status already updated by user, if you wish to make changes, please contact Admin.",
        code: 401,
        data: [],
      }),
      headers: {
...
async function getMeetings(requestBody) {
  ...
  const result = await knex
    ...
    .leftJoin(
      MEETING_ACTIVITY_LOG,
      MEETING_ACTIVITY_LOG + ".meeting_id",
      MEETING + ".meeting_id"
    )
    .leftJoin(
      USER + " AS meetingUser",
      MEETING_ACTIVITY_LOG + ".user_id",
      "meetingUser.user_id"
    )
    .leftJoin(USER, PARTICIPANTS + ".user_id", USER + ".user_id")
    .leftJoin(
      MEETING_BLOCK_TIME,
      MEETING_BLOCK_TIME + ".meeting_id",
      MEETING + ".meeting_id"
    )
  ...
  if (result.length > 0) {
    ...
    result.forEach((ele) => {
      ...
      var obj = {
        ...
        meeting_description: ele.meeting_description
          ? ele.meeting_description
          : "",
        ...
        scheduled_start_time: ele.scheduled_start_time
          ? ele.scheduled_start_time
          : "",
        scheduled_end_time: ele.scheduled_end_time
          ? ele.scheduled_end_time
          : "",
      ...
      var index = meetingDataFinal.findIndex(
        (x) => x.meeting_id == ele.meeting_id
      );
      ...
      var participateObj = {
        meeting_participant_id: ele.meeting_participant_id
          ? ele.meeting_participant_id
          : "",
        meeting_id: ele.meeting_id ? ele.meeting_id : "",
      ...
      if (participateIndex == -1) {
        ...
        if (ele.meeting_participant_id != null) {
          console.log(
            "coming in participate push" + ele.meeting_participant_id
          );
      ...
      var activityObj = {
        meeting_activity_log_id: ele.meeting_activity_log_id
          ? ele.meeting_activity_log_id
          : "",
        meeting_id: ele.meeting_id ? ele.meeting_id : "",
        user_id: ele.user_id ? ele.user_id : "",
        proposed_meeting_date: ele.proposed_meeting_date
          ? ele.proposed_meeting_date
          : "",
        proposed_start_time: ele.proposed_start_time
          ? ele.proposed_start_time
          : "",
      ...
      if (activityIndex == -1) {
        if (ele.meeting_activity_log_id != null) {
          console.log(
            "coming in participate push" + ele.meeting_activity_log_id
          );
  ...
  if (result.length > 0) {
    ...
    result.forEach((ele) => {
      ...
      var obj = {
        ...
        meeting_location: ele.meeting_location ? ele.meeting_location : "",
        meeting_duration: ele.meeting_duration ? ele.meeting_duration : "",
        meeting_min_duration: ele.meeting_min_duration
          ? ele.meeting_min_duration
          : "",
        meeting_max_duration: ele.meeting_max_duration
          ? ele.meeting_max_duration
          : "",
  ...
  if (result.length > 0) {
    ...
    result.forEach((ele) => {
      ...
      var obj = {
        ...
        client_id:
          "923376702287-vk1qrn95vda3s6k9053n3licg2pdcl7d.apps.googleusercontent.com",
...
async function getBlockDataForMeeting(meetingId) {
  ...
  // console.log(meetingrpId);
  const result = await knex
    .select(MEETING_CALENDAR + ".*", USER + ".name", USER + ".email")
  ...
  if (result.length > 0) {
    ...
    if (emailStatus) {
      ...
      let returnData = {
        body: JSON.stringify({
          message:
            "Unable to send the access request at this moment, please contact Admin.",
          code: 400,
          data: [],
        }),
        headers: {
...
async function sendAccessEmail(email, accesslink) {
  ...
  const compiledTemplate = accessEmailTemplate
...
async function handleMeetingStatus(meetingScheduleResponse, iteration = 0) {
  ...
  if (
    meetingScheduleResponse.status == 2 ||
    meetingScheduleResponse.status == "2"
  ) {
    //Meeting is scheduled
    let updateData = {
      meeting_status: meetingScheduleResponse.status,
      scheduled_date: moment(meetingScheduleResponse.date, "DD/MM/YYYY").format(
        "YYYY-MM-DD"
      ),
    ...
    if (meetingScheduleResponse.logData) {
      ...
      await saveBlockTimeDataOfUser(
        meetingScheduleResponse.meetingId,
        meetingScheduleResponse
      );
    }
    await notifyAdmin(meetingScheduleResponse.meetingId, 1);
  } else if (
    meetingScheduleResponse.status == 1 ||
    meetingScheduleResponse.status == "1"
  ) {
    ...
    const meetingString =
      `Meeting Date: ${meetingScheduleResponse.date} \n` +
    ...
    if (iteration <= 4) {
      for (const item of meetingScheduleResponse.sendEmail) {
        try {
          ...
          let activityId = await insertActivityLog(
            meetingScheduleResponse,
            item.user_id
          );

          let emailSend = await sendTimeSlotEmail(
            item.email,
            item.user_id,
            activityId,
            meetingString
          ...
          console.log(
            `Inserted email ${activityId} for meeting ${meetingScheduleResponse.meetingId}`
          );
...
async function insertActivityLog(requestBody, userId) {
  ...
  let meetingObject = {
    ...
    user_id: userId,
    proposed_meeting_date: moment(requestBody.date, "DD/MM/YYYY").format(
      "YYYY-MM-DD"
    ),
  ...
  try {
    let [insertId] = await knex(MEETING_ACTIVITY_LOG)
      .insert(meetingObject)
      .returning("meeting_activity_log_id");
    console.log("userInsert");
...
async function sendTimeSlotEmail(
  email,
  userId,
  meetingActivityId,
  meetingString
  ...
  console.log("email in sendTimeSlotEmail function:-----------");
  console.log(email);
  console.log("userId in sendTimeSlotEmail function:-----------");
  console.log(userId);
  console.log("meetingActivityId in sendTimeSlotEmail function:-----------");
  console.log(meetingActivityId);
  console.log("meetingString in sendTimeSlotEmail function:-----------");
  console.log(meetingString);
  console.log("BREAK:------------")
  let username = "";
  let accessLinkApprove =
    process.env.API_URL +
    "/saveCurrentSelectionForMeeting?activityLogId=" +
    meetingActivityId +
    "&userId=" +
    userId +
    "&status=1";
  let accessLinkReject =
    process.env.API_URL +
    "/saveCurrentSelectionForMeeting?activityLogId=" +
    meetingActivityId +
    "&userId=" +
  ...
  try {
    const compiledTemplate = timeSlotEmailTemplate
      .replace("<%= username %>", username)
...
async function updateMeetingDetails(meetingId, data) {
  ...
  try {
    ...
    if (data.scheduled_start_time)
      updateObject.scheduled_start_time = data.scheduled_start_time;
    if (data.scheduled_end_time)
      updateObject.scheduled_end_time = data.scheduled_end_time;
    ...
    let meetingUpdate = await knex(MEETING)
      .update(updateObject)
...
async function updateMeetingLog(meetingActivityId, data) {
  ...
  try {
    ...
    let meetingUpdate = await knex(MEETING_ACTIVITY_LOG)
      .update(updateObject)
      .where("meeting_activity_log_id", meetingActivityId);
    console.log("userInsert");
    console.log(meetingUpdate);
    if (meetingUpdate) {
      await knex.destroy();
...
async function deleteMeetingCalendarRecordsFromDB(meetingId) {
  ...
  try {
    let meetingUpdate = await knex(MEETING_CALENDAR)
      .delete()
      .where("meeting_id", meetingId);
    console.log("userInsert");
    console.log(meetingUpdate);
...
async function saveBlockTimeDataOfUser(meetingId, data) {
  ...
  try {
    let meetingUpdate = await knex(MEETING_BLOCK_TIME)
      .delete()
      .where("meeting_id", meetingId);
    let blockJson = {
    ...
    let blockDataInsert = await knex(MEETING_BLOCK_TIME).insert(
      blockDataObject
    );
    console.log("userInsert");
...
async function deleteBlockTimeSlot(blockTimeID) {
  ...
  try {
    const participantEmails = blockTimeID.map(
      (participant) => participant.email
    );
    const calendarIds = participantEmails.flat();
    ...
    for (let i = 0; i < blockTimeID.length; i++) {
      ...
      if (userClient) {
        ...
        try {
          ...
          console.error(
            `Error deleting event for user ${organizerEmail}:`,
            error
          );
...
// FUNCTION TO GET THE BEST SUITABLE TIME FOR PARTICIPANTS ---START---
async function getTimeSlots(meetingData) {
  try {
    ...
    // if (jsonData.hasOwnProperty('is_preview')) {
    //   // If is_preview key exists, set its value to 1
    //   is_preview = 1;
    // }
    ...
    if(is_preview === 1){
    const selectedPreview = await selectedPreviewTime(meetingData)
    }
...
// FUNCTION TO GET THE BEST SUITABLE TIME FOR PARTICIPANTS ---END---
...
// FUNCTION WHEN ADMIN SELECT SUITABLE TIME FROM CMS FOR PARTICIPANTS ---START---
async function selectedPreviewTime(meetingData) {

  console.log("selectedPreviewTime function HIT:-----------")
  ...
  try {
    ...
    // selected_slot with all users not available
    // const selected_slot = [
    //   {
    //     commonSlot: {
    //       start: "2024-07-19T06:30:00.000Z",
    //       end: "2024-07-19T07:00:00.000Z",
    //     },
    //     available: [
    //       { email: "romanl341@outlook.com", user_id: 44, name: "Roman Lee" },
    //     ],
    //     unavailable: [
    //       { email: "anayaa341@outlook.com", user_id: 48, name: "Anaya Andrew" },
    //       { email: "tamarah341@outlook.com", user_id: 45, name: "Tamara Hope" }
    //     ],
    //   }
    // ]
    ...
    const selected_slot = [
      {
        commonSlot: {
          start: "Moment<2024-07-19T08:30:00+02:00>",
          end: "Moment<2024-07-19T09:30:00+02:00>"
        },
        available: [
          { email: "romanl341@outlook.com", user_id: 44, name: "Roman Lee" },
        ],
    ...
    // selected_slot with all users available
    // const selected_slot = [
    //   {
    //     commonSlot: {
    //       start: "Moment<2024-07-20T08:30:00+02:00>",
    //       end: "Moment<2024-07-20T09:00:00+02:00>"
    //     },
    //     available: [
    //       { email: "tamarah341@outlook.com", user_id: 45, name: "Tamara Hope" },
    //       { email: "romanl341@outlook.com", user_id: 44, name: "Roman Lee" },
    ...
    const availableList = selected_slot.map((element) => {
      return element.available
    })

    let availableListFlat = availableList.flat()

    const availableListArray = availableListFlat.map((element) => {
      return element.email
    })

    console.log("availableListArray in selectedPreviewSlot function:----------")
    console.log(availableListArray)



    const sendList = selected_slot.map((element) => {
      return element.unavailable
    })

    let sendListFlat = sendList.flat()

    const sendListArray = sendListFlat.map((element) => {
      return element.email
    })

    console.log("sendListArray in selectedPreviewSlot function:----------")
    console.log(sendListArray)

    let slotStartTZ = null;
    let slotEndTZ = null;

    for (const slot of selected_slot) {
      if (slot.commonSlot) {
        slotStartTZ = slot.commonSlot.start;
        slotEndTZ = slot.commonSlot.end;
        break; // Exit the loop once commonSlot is found
      }
    }
    
    
    console.log("slotStartTZ in selectedPreviewSlot function:----------")
    console.log(slotStartTZ)
    console.log("slotEndTZ in selectedPreviewSlot function:----------")
    console.log(slotEndTZ)


    let momentStr = slotStartTZ;
    // Extract the actual dateTime part
    let dateTimeStr = momentStr.match(/Moment<([^>]+)>/)[1];
    // Parse the extracted time string and convert to UTC
    let utcMoment = moment(dateTimeStr).utc();
    // Format the dateTime object to the desired string format
    let slotStart = utcMoment.format("YYYY-MM-DDTHH:mm:ss.SSS[Z]");


    let momentStrEnd = slotEndTZ;
    // Extract the actual dateTime part
    let dateTimeStrEnd = momentStrEnd.match(/Moment<([^>]+)>/)[1];
    // Parse the extracted time string and convert to UTC
    let utcMomentEnd = moment(dateTimeStrEnd).utc();
    // Format the dateTime object to the desired string format
    let slotEnd = utcMomentEnd.format("YYYY-MM-DDTHH:mm:ss.SSS[Z]");

    console.log("slotStart in selectedPreviewSlot function:----------")
    console.log(slotStart)
    console.log("slotEnd in selectedPreviewSlot function:----------")
    console.log(slotEnd)
    ...
    if(sendListArray.length == 0 && participants.length == availableListArray.length){
      ...
      const scheduleTimeStart = moment.tz(slotStart, meeting_timeZone).format('HH:mm');
      const scheduleTimeEnd = moment.tz(slotEnd, meeting_timeZone).format('HH:mm');
      const scheduleDate = moment.tz(slotStart, meeting_timeZone).format('DD/MM/YYYY');

      const previewMeetingScheduled = {
      ...
      console.log("previewMeetingScheduled:----------")
      console.log(previewMeetingScheduled)


      return previewMeetingScheduled;
      ...
      console.log("meetingUserIds in selectedPreviewSlot function in else condition:----------")
      console.log(meetingUserIds)

      let amsterdamTimeZone = meeting_timeZone;
      ...
      const firstSlotStartAmsterdam = convertToAmsterdamTime(slotStart);
      const meetingEndTimeAmsterdam = convertToAmsterdamTime(slotEnd);
  
      const previewCalendarIds = calendarIds.filter((id) => !sendListArray.includes(id));
      ...
      const start_time = moment.tz(slotStart, meeting_timeZone).format("HH:mm");
      const end_time = moment.tz(slotEnd, meeting_timeZone).format("HH:mm");
      const date = moment.tz(slotStart, meeting_timeZone).format("DD/MM/YYYY");
  
      const previewEmailSend = {
      ...
      console.log("previewEmailSend:-------");
      console.log(previewEmailSend);


      const meetingString =
      `Meeting Date: ${previewEmailSend.date} \n` +
      `Start Time: ${previewEmailSend.startTime} \n` +
      `End Time: ${previewEmailSend.endTime}`;


        for (const record of previewEmailSend.sendEmail) {
        try {
          // Perform database insertion for each record
          previewEmailSend.iteration = iteration;
          let activityId = await insertActivityLog(
            previewEmailSend,
            record.user_id
          );
          ...
          console.log(
            `Inserted email ${activityId} for meeting ${previewEmailSend.meetingId}`
          );
          console.log(`Email send response ${emailSend} `);
        } catch (error) {
          console.error(`Error inserting email ${record.email}:`, error);
        }
      }
      return previewEmailSend


    }
  } catch (error) {
    console.log(`Error in selectedPreviewTime function ${error}`);
    return { message: "Something went wrong", status: 400 };
  }
}
// FUNCTION WHEN ADMIN SELECT SUITABLE TIME FROM CMS FOR PARTICIPANTS ---END---
...
async function refreshTokenIfNeeded(
  ...
  if (expireTimestamp < currentTimestamp) {
    ...
    let userUpdate = await updateUser(userEmail, {
      access_token: access_token,
      token_expires_at: expireDate,
    });
    ...
    console.log(
      `Access Token is not Expired for ${userEmail}, using the existing one:------------`
    );
...
*/
    ...
    console.log("is_preview outside preview:---------")
    console.log(is_preview)



    // CONDITION TO SHOW PREVIEW ---START---
    if(is_preview === 1){
      console.log("Show preview HIT:---------")
      // console.log("is_preview in preview:---------")
      // console.log(is_preview)
      ...
      // console.log("commonFreeSlots when we have to show preview:-----------")
      // console.log(commonFreeSlots)
      // const CFS = JSON.stringify(commonFreeSlots)
      // console.log(CFS)
      // console.log("outOfOfficeEvents when we have to show preview:-----------")
      // console.log(outOfOfficeEvents)


      // Function to adjust commonFreeSlots based on oof events
      const adjustCommonFreeSlots = (slots, oofEvents, timeZone) => {
      return slots.flatMap(slot => {
      const { commonSlot, sendList } = slot;
      let adjustedSlots = [{ ...commonSlot }];

      sendList.forEach(email => {
      const oofEvent = oofEvents.find(event => event.scheduleId === email);

      if (oofEvent) {
        const oofStart = moment.tz(oofEvent.start, timeZone).valueOf();
        const oofEnd = moment.tz(oofEvent.end, timeZone).valueOf();
        
        adjustedSlots = adjustedSlots.flatMap(slot => {
          const commonStart = moment.tz(slot.start, timeZone).valueOf();
          const commonEnd = moment.tz(slot.end, timeZone).valueOf();
          
          if (oofStart >= commonEnd || oofEnd <= commonStart) {
            // No overlap
            return [slot];
          } else if (oofStart <= commonStart && oofEnd >= commonEnd) {
            // OOF completely covers the common slot
            return [];
          } else if (oofStart <= commonStart) {
            // OOF overlaps the start of the common slot
            return [{ start: moment.tz(oofEnd, timeZone), end: slot.end }];
          } else if (oofEnd >= commonEnd) {
            // OOF overlaps the end of the common slot
            return [{ start: slot.start, end: moment.tz(oofStart, timeZone) }];
          } else {
            // OOF is in the middle of the common slot
            return [
              { start: slot.start, end: moment.tz(oofStart, timeZone) },
              { start: moment.tz(oofEnd, timeZone), end: slot.end }
            ];
          }
        });
      }
    ...
    return adjustedSlots.map(newSlot => ({
      ...slot,
      commonSlot: newSlot
    }));
  });
...
// let suitableFreeSlot = adjustCommonFreeSlots(commonFreeSlots, outOfOfficeEvents, meeting_timeZone);
let suitableFreeSlot = commonFreeSlots ? adjustCommonFreeSlots(commonFreeSlots, outOfOfficeEvents, meeting_timeZone) : null;


// console.log("suitableFreeSlot after removing the OOF when we have to show preview:-----------")
// console.log(suitableFreeSlot)
// const ACFS = JSON.stringify(suitableFreeSlot)
// console.log(ACFS)
// console.log("jsonData after removing the OOF when we have to show preview:-----------")
// console.log(jsonData)
    ...
    // Assuming commonFreeSlots is your original array

// Assuming commonFreeSlots is your original array
let suitableSlots = [];

// Iterate through each group in commonFreeSlots
if (suitableFreeSlot) {
suitableFreeSlot.forEach(group => {
    // Extract commonSlot for each group
    const commonSlot = group.commonSlot;

    // Check if commonSlot exists and meets the minimum meeting duration requirement
    if (commonSlot && moment.duration(moment(commonSlot.end).diff(commonSlot.start)).asMilliseconds() >= minimumMeetingDuration) {
        // Filter free minutes within the group that meet the minimum duration requirement
        const filteredGroup = {
            group: group.group.map(user => ({
                name: user.name,
                freeMinutes: user.freeMinutes.filter(minutes => {
                    const slotDuration = new Date(minutes.end).getTime() - new Date(minutes.start).getTime();
                    return slotDuration >= minimumMeetingDuration;
                })
            })),
            commonSlot: commonSlot,
            totalFree: group.totalFree, // Include totalFree from the original group
            sendList: group.sendList // Include sendList from the original group
        };

        // Add to suitableSlots only if there are suitable free minutes
        if (filteredGroup.group.some(user => user.freeMinutes.length > 0)) {
            suitableSlots.push(filteredGroup);
        }
    }
});
} else {
...
// Now suitableSlots contains filtered data while maintaining the original structure
    // console.log("suitableSlots in preview condition:------------");
    // console.log(suitableSlots);
    // const SS = JSON.stringify(suitableSlots)
    // console.log(SS)
    ...
    if (suitableSlots.length > 0) {
      ...
      suitableSlots.forEach(slot => {
        const { start, end } = slot.commonSlot;
        const slotStart = new Date(start);
        const slotEnd = new Date(end);
      
        let currentStart = new Date(slotStart);
        
        while ((slotEnd - currentStart) >= minimumMeetingDuration) {
          const meetingEndTime = new Date(currentStart.getTime() + minimumMeetingDuration);
          
          newSlots.push({
            group: slot.group,
            commonSlot: {
              // start: new Date(currentStart).toISOString(),
              // end: new Date(meetingEndTime).toISOString()
              start: moment.tz(currentStart, timeZone),
              end: moment.tz(meetingEndTime, timeZone)
            },
            totalFree: slot.totalFree,
            sendList: slot.sendList
          });
          
          currentStart = new Date(meetingEndTime); // move to the next potential slot start
          // currentStart = moment.tz(meetingEndTime, timeZone); // move to the next potential slot start
        }
      });

    }

    // console.log("newSlots in preview condition:------------");
    // console.log(newSlots);
    // const NS = JSON.stringify(newSlots)
    // console.log(NS)
    // console.log("participants in preview condition:-----------")
    ...
    const processedSlots = newSlots.map(slot => {
      const availableParticipants = slot.group.map(groupMember => {
        const participant = participants.find(p => p.email === groupMember.name);
        return participant ? {
      ...
      const unavailableParticipants = slot.sendList.map(email => {
      ...
      return {
        commonSlot: slot.commonSlot,
        available: availableParticipants,
        unavailable: unavailableParticipants,
      };
    });
    
    // console.log("processedSlots in preview condition:------------");
    // console.log(processedSlots);
    // const PS = JSON.stringify(processedSlots)
    // console.log(PS)

    let previewResult = {
      previewSlots: processedSlots,
      logData: logData.enrichedSlotsByDateWithTime,
      commonTimeSlots: commonTimeSlots,
    }

    console.log("previewResult in preview condition:------------");
    console.log(previewResult);
    const PR = JSON.stringify(previewResult)
    console.log(PR)

      return previewResult

    }
    // CONDITION TO SHOW PREVIEW ---END---
    ...
    async function getCommonFreeSlots(
      startTime,
      endTime,
      busySlotsArray,
      scheduleJsonData,
    ...
    async function findSuitableSlot(
      startTime,
      endTime,
      busySlotsArray,
      scheduleJsonData,
      ...
      // console.log(
      //   "commonFreeSlots in findSuitableSLot function FIRST:------------"
      // );
      // console.log(commonFreeSlots);
      ...
      while (commonFreeSlots !== null && iterationCount < maxIterations) {
        ...
        if (
          ...
          if (
            ...
            minimumMeetingDuration =
              moment
                .duration(
                  moment(suitableFreeSlot[0].end).diff(
                    suitableFreeSlot[0].start
                  )
                )
                .asMinutes() *
              60 *
        ...
        let suitableSlots = suitableFreeSlot
          ? suitableFreeSlot.filter((slot) => {
              const slotDuration =
        ...
        for (const userToCheck of meetingSuitableTime.suitableSendListArray) {
          const userOOFEvents = outOfOfficeEvents.filter(
            (event) =>
              event.scheduleId === userToCheck && event.status === "oof"
          );
          for (const userOOFEvent of userOOFEvents) {
            const oofStart = new Date(userOOFEvent.start);
            const oofEnd = new Date(userOOFEvent.end);
            if (
              (suitableStart >= oofStart && suitableStart < oofEnd) ||
              (suitableEnd > oofStart && suitableEnd <= oofEnd)
            ) {
              console.log(
                `User ${userToCheck} is OOF, finding new slot...........`
              );
      ...
      console.log(
        "Failed to find a suitable meeting time after multiple attempts."
      );
...
async function calculateCommonFreeSlotsGoogle(
  startTime,
  endTime,
  busySlots,
  jsonData
...
async function calculateCommonFreeSlots(
  ...
  try {
    ...
    // console.log("commonFreeSlots in calculateCommonFreeSlots function when we have to show preview:-----------")
    // console.log(commonFreeSlots)
...
async function findCommonFreeSlots(
  start,
  end,
  busyPeriods,
  jsonDataFormate,
  ...
  try {
    ...
    for (let element of datesInBetween) {
      ...
      if (data.meeting_timing == 4) {
        ...
        const time2 = moment(data.end_time, "HH:mm:ss").tz(
          data.meeting_timeZone
        );
      ...
      // RETURN SINGLE SLOT HANDLING ---START---
  //     if(is_preview === 0){
  //     if (result) {
...
function findCommonTimeForMulti(arr) {
  ...
  return minArr.length ? minArr[0] : minArr;
    ...
    // RETURN SINGLE SLOT HANDLING ---END---
    ...
    function secondIterationHandling(
      ...
      for (let log of meeting_log) {
        let datePart = moment(log.proposed_meeting_date, "YYYY-MM-DD").tz(
          meeting_timeZone
        );
        let flagForMulti =
          datePart.isSame(commonStart, "day") ||
          datePart.isSame(commonEnd, "day");

        let startTime = moment(log.proposed_start_time, "HH:mm:ss").tz(
          meeting_timeZone
        );
        let endTime = moment(log.proposed_end_time, "HH:mm:ss").tz(
          meeting_timeZone
        );

        let mergedStartTime = datePart
          .clone()
        ...
        let diffFor2it = moment
          .duration(commonEnd.diff(mergedEndTime))
          .asMinutes();
        let diffFor2it1 = moment
          .duration(mergedStartTime.diff(commonStart))
          .asMinutes();

        const isOverlap =
          (commonStart.isSameOrBefore(mergedStartTime) &&
            commonEnd.isSameOrAfter(mergedStartTime)) ||
          (commonStart.isSameOrBefore(mergedEndTime) &&
            commonEnd.isSameOrAfter(mergedEndTime)) ||
          (mergedStartTime.isSameOrBefore(commonStart) &&
            mergedEndTime.isSameOrAfter(commonEnd));
    ...
    function getTime(
          ...
          for (let i = 0; i < userSlots.length; i++) {
            ...
            if (i == userSlots.length - 1) {
              ...
              if (totalFreeTime > 0) {
                user["freeslots"].push({
                  start: moment(userSlots[i].end).tz(data.meeting_timeZone),
                  end: moment(userSlots[i + 1]?.start).tz(
                    data.meeting_timeZone
                  ),
                });
                user["freeslotsinmin"].push(totalFreeTime);
          ...
          commonTimeForMax.maxUserGroup.forEach((x) => {
            if (x.group.length == 1) {
              x.group[0].freeMinutes.forEach((ele) => {
                ...
                if (evening) {
                  console.log(
                    "commonStart in evening before condition in ALGO:-------"
                  );
                  console.log(commonStart);
                  console.log(
                    "commonEnd in evening before condition in ALGO:-------"
                  );
      ...
      let findClosestTime = (arr) => {
        ...
        if (evening) {
          if (
            closestTimeSlot.start.isSameOrAfter(
              moment(newMorningEveningTime)
            ) &&
...
async function authorizeGoogle(calendarIds) {
  ...
  let redirect_uris = [
    "https://ebmrnxlpk9.execute-api.eu-central-1.amazonaws.com/dev/oauth2callback",
  ];
  ...
  try {
    ...
    function blockTimeData(participants, userIDOrganizer) {
      return userIDOrganizer
        ...
        .filter((userInfo) =>
          participants.some(
            (participant) => participant.email === userInfo.organizerEmail
          )
        )
        .map((userInfo) => ({
    ...
    const blockTimeID = userIDOrganizer
      .filter((user) =>
        participants.some(
          (participant) =>
            participant.email.toLowerCase() == user.organizerEmail.toLowerCase()
        )
      )
      .map((user) => ({
        calendar_meeting_id: user.meetingId,
...
async function outh2CallBackGoogle(data) {
  ...
  try {
    ...
    if (tokens.tokens.refresh_token) {
      let userUpdate = await updateUser(state, {
        access_status: 3,
        is_verified: 1,
...
async function outh2CallBackPassport(data) {
  ...
  try {
    ...
    if (tokens.refreshToken && tokens.accessToken && email) {
      ...
      expireDate.toISOString();
      let userUpdate = await updateUser(email, {
        access_status: 3,
        is_verified: 1,
...
async function updateUser(emailId, data) {
  ...
  try {
    ...
    if (data.token_expires_at)
      updateObject.token_expires_at = data.token_expires_at;

    let meetingUpdate = await knex(USER)
      .update(updateObject)
      .where("email", emailId);
    console.log("userInsert");
    console.log(meetingUpdate);
...
async function notifyAdmin(meetingId, type) {
  ...
  try {
    ...
    const meetingString =
      `Start Time: ${meetingData.scheduled_start_time} \n` +
      `End Time: ${meetingData.scheduled_end_time}`;

    if (type == 1) {
      compiledTemplate = meetingConfirmationEmailTemplate
        .replace("<%= username %>", username)
        .replace("<%= meetingTime %>", meetingString)
        .replace("<%= meetingName %>", meetingData.meeting_title)
        .replace("<%= meetingDate %>", meetingData.scheduled_date);
    } else if (type == 2) {
      compiledTemplate = meetingFailedEmailTemplate
        .replace("<%= username %>", username)
        .replace("<%= meetingName %>", meetingData.meeting_title)
  ...
  try {
    ...
    enrichedSlotsByDate.map((dateSlot) => {
      dateSlot.slots = dateSlot.slots.map((slot) => {
        slot.times = updateTimesWithStatus(
          slot.times,
          freeTentativeWEWEvents,
          slot.email
        );
        return slot;
      });
...
async function findCommonFreeTimeSlots(
  logData,
  meeting_timeZone,
  jsonData,
  currentTime
  ...
  try {
    ...
    logData.forEach((day) => {
      const date = day.date;
      const userSlots = day.slots.map(
        (userSlot) => Object.entries(userSlot)[0]
      );
    ...
    const commonFreeSlotsWithNameAndID = commonFreeSlots.map((slot) => {
      return {
        ...
        users: slot.users.map((email) => {
          const participant = participantsMap[email];
          return participant
            ? { email, user_id: participant.user_id, name: participant.name }
            : { email };
        }),
      };
    });
